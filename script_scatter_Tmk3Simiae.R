...
heat[,"SumSpend"]= rowSums(heat[c("L", "O")])/2
heat[,"control"]=rowSums(heat[,c("D","E")])/2

genesFitTmk3down=rownames(heat)[heat$SumSpend < -1 & heat$control > -1]
genesFitTmk3up=rownames(heat)[heat$SumSpend > 1.5 & heat$control < 1]
genesFitComple=rownames(heat)[heat$SumSpend < 1 & heat$control < -1.5& heat$SumSpend  > -1]

length(genesFitTmk3down)
length(genesFitTmk3up)
length(genesFitComple)

fitneneg=heat[genesFitTmk3down,]
fitneposi=heat[genesFitTmk3up,]
...
plot( heat$SumSpend, heat$control,  col="darkgray", xlim = c(-4,2), ylim = c(-4,1), cex = 0.7,pch=19, 
      xlab = "T. atroviride Tmk3-Spent media", ylab= "Without Fungus", main = "Pseudomonas simiae WCS417")
...
legend(0, -2.5, legend=c("Negative Fitness","Recovered","Fusaric acid resistance", "ABC", "Arn-operon"),
