...
genesFitdownWT=rownames(heat)[heat$SumSpend < -1 & heat$control > -1]
genesFitupWT=rownames(heat)[heat$SumSpend > 1 & heat$control < 1]
genesFitComple=rownames(heat)[heat$SumSpend < 1 & heat$control < -1.5 & heat$SumSpend  > -1]

length(genesFitdownWT)
length(genesFitupWT)
...
plot( heat$SumSpend, heat$control,  col="darkgray", xlim = c(-5,2), ylim = c(-5,1), cex = 0.7,pch=19, 
      xlab = "T. atroviride WT-Spent media", ylab= "Without Fungus", main = "Pseudomonas putida")
